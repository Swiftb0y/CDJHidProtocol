...
https://en.wikipedia.org/wiki/Challenge%E2%80%93response_authentication[Challege-Response-Protocol]
...
The engineers implemented their own one-way hash function which is not very
complicated. Here is a complete reference implementation in python3.8:

[source, python]
----
def hash(buff: bytes) -> int:
  mask = 0x811c9dc5
  for val in buff:
    mask = ((val^mask) * 0x1000193) & 0xFFFFFFFF
  return mask
----
...
secret and `SeedE` to each other and hashing that: `hashAd = hash(seedA+secret)`
