...
(tlv-struct 5 0x16
  #(draw-related-boxes [0x00 0x02 0x0f 0x00 0x0f 0x04 0x05 0x01 0x00 0x0e 0x0d 0x03 0x00 0x0a 0x02 0x0c]))

(draw-padding 0x30)
----

On closer examination, we can see that this packet is transferring
a buffer which is larger than the 42 possible bytes in a single packet 
so it is followed up by a packet with rest of the data.
...
(draw-related-boxes [0x01 0x00 0x04 0x02])
